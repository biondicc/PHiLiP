#include "hyper_DWR_LS_sampling_error_updated.h"
#include <iostream>
#include <filesystem>
...
HyperDWRLSSamplingErrorUpdated<dim, nstate>::HyperDWRLSSamplingErrorUpdated(const PHiLiP::Parameters::AllParameters *const parameters_input,
...
template <int dim, int nstate>
int HyperDWRLSSamplingErrorUpdated<dim, nstate>::run_sampling() const
{
    ...
    this->pcout << "Create LS problem..."<< std::endl;
    HyperReduction::DWRLSWeights<dim,nstate> DWR_LS_weights_solver(this->all_parameters, this->parameter_handler, constructer_NNLS_problem->A->trilinos_matrix(), Comm, b_Epetra, adj_weights.weights);
    this->pcout << "Solve LS problem..."<< std::endl;
    DWR_LS_weights_solver.solve();

    ptr_weights = std::make_shared<Epetra_Vector>(DWR_LS_weights_solver.weights);
...
template <int dim, int nstate>
void HyperDWRLSSamplingErrorUpdated<dim, nstate>::outputIterationData(std::string iteration) const{
...
template <int dim, int nstate>
RowVectorXd HyperDWRLSSamplingErrorUpdated<dim, nstate>::getMaxErrorROM() const{
...
template <int dim, int nstate>
void HyperDWRLSSamplingErrorUpdated<dim, nstate>::placeInitialSnapshots() const{
...
template <int dim, int nstate>
bool HyperDWRLSSamplingErrorUpdated<dim, nstate>::placeROMLocations(const MatrixXd& rom_points, Epetra_Vector weights) const{
...
template <int dim, int nstate>
void HyperDWRLSSamplingErrorUpdated<dim, nstate>::updateNearestExistingROMs(const RowVectorXd& /*parameter*/, Epetra_Vector weights) const{
    std::unique_ptr<FlowSolver::FlowSolver<dim,nstate>> flow_solver = FlowSolver::FlowSolverFactory<dim,nstate>::select_flow_case(all_parameters, parameter_handler);
...
template <int dim, int nstate>
dealii::LinearAlgebra::distributed::Vector<double> HyperDWRLSSamplingErrorUpdated<dim, nstate>::solveSnapshotFOM(const RowVectorXd& parameter) const{
...
template <int dim, int nstate>
std::unique_ptr<ProperOrthogonalDecomposition::ROMSolution<dim,nstate>> HyperDWRLSSamplingErrorUpdated<dim, nstate>::solveSnapshotROM(const RowVectorXd& parameter, Epetra_Vector weights) const{
...
template <int dim, int nstate>
Parameters::AllParameters HyperDWRLSSamplingErrorUpdated<dim, nstate>::reinitParams(const RowVectorXd& parameter) const{
    // Copy all parameters
...
template <int dim, int nstate>
void HyperDWRLSSamplingErrorUpdated<dim, nstate>::configureInitialParameterSpace() const
{
...
#if PHILIP_DIM==1
        template class HyperDWRLSSamplingErrorUpdated<PHILIP_DIM, PHILIP_DIM>;
#endif

#if PHILIP_DIM!=1
        template class HyperDWRLSSamplingErrorUpdated<PHILIP_DIM, PHILIP_DIM+2>;
#endif
